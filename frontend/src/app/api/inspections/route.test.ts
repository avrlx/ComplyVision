...
vi.mock("@/lib/auth/config", () => ({ databaseEnabled: mocks.enabled, supabaseConfigured: () => true, testingMode: () => false, testingDatabaseConfigured: () => false, testingWorkspaceUserId: () => undefined }));
vi.mock("@/lib/supabase/server", () => ({ createClient: async () => ({ auth: { getUser: mocks.user }, from: mocks.from }) }));
vi.mock("@/lib/supabase/admin", () => ({ createTestingAdminClient: vi.fn() }));
