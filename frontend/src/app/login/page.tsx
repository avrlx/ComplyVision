...
export default function LoginPage() {
  ...
  async function sendPrimaryOtp() {
    ...
    if (identifierMode === "email") {
      ...
      setMessage(`We sent a sign-in link or one-time code to ${email.trim().toLowerCase()}.`);
    } else {
      throw new Error("Phone sign-in is not available yet. Use email OTP.");
    }
        ...
        <div className="w-full rounded-2xl border border-slate-200 bg-white p-7 shadow-xl shadow-slate-200/50">
          ...
          {step === "credentials" && (
            <div className="mb-5 grid grid-cols-2 rounded-lg bg-slate-100 p-1">
              ...
              <button type="button" disabled aria-disabled="true" className="cursor-not-allowed rounded-md px-3 py-2 text-sm font-semibold text-slate-400">Phone (coming soon)</button>
            </div>
          )}
